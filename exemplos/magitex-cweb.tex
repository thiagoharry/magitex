...
\cor{0 0 0}{OI!}Código bonito geralmente é simples---claro e fácil de entender. Código
...
A função \monoespaco{match(regexp, text)} testa se há uma ocorrência
...
A operação básica de \monoespaco{match} é direta. Se o primeiro
...
laço \monoespaco{do-while}, algo relativamente incomum na construção
de programas em C.A ocorrência de um \monoespaco{do-while} deve sempre
...
O grosso do trabalho é feito na função \monoespaco{matchhere(regexp,
  text)}, a qual testa se a expressão regular casa com o texto que
começa logo ali. A função opera tentando casar o primeiro caractere da
expressão regular com o primeiro caractere de texto. Se o casamento
falhar, não pode haver casamento nesta posição do texto e
\monoespaco{matchhere} retorna 0. Se o casamento for bem-sucedido,
contudo, é possível avançar para o próximo caractere da expressão
regular e o próximo caractere do texto. Isso é feito chamando
\monoespaco{matchhere} recursivamente.
...
(\monoespaco{regexp[0] == '\\ 0'}), então todos os testes anteriores
foram bem-sucedidos, e assim a expressão regular casa com o texto.

Se uma expressão regular é um caracere seguido por *,
\monoespaco{matchstar} é chamada para ver se a palavra casa com o
fechamento. A função \monoespaco{matchstar(c, regexp, text)} tenta
...
variante de \monoespaco{matchstar} para este caso, mostrada depois.
...
\monoespaco{matchhere}. Esta chamada recursiva é o coração do
...
neste ponto na expressão regular, e então \monoespaco{matchhere}
retorna 0.
...
\monoespaco{matchhere} para lidar primeiro com \$ antes de *. Embora
não faça diferença aqui, me parece mais natural, e uma boa regra é
cuidar dos casos simples antes dos mais difíceis.

Mas via de regra, contudo, a ordem dos testes é crítica. Por exemplo,
neste teste de \monoespaco{matchstar}:
...
\monoespaco{text++} deve sempre ser feito.
...
Esta versão de \monoespaco{matchstar} que implementa o maior casamento
mais à esquerda começa identificando uma sequencia máxima de
ocorrências do caractere de entrada ``c''. Então, ela usa
\monoespaco{matchhere} para tentar extender o casamento para o resto
dos padrões e o resto do texto. Cada falha reduz o número de ``c''s em
um e tenta-se novamente, incluindo o caso de zero ocorrências.
...
natural seria substituir os \monoespaco{char *}s do código original
por uma estrutura:
...
(4) Use algum tipo opaco para esconder a estrutura \monoespaco{RE} e
...
regulares mas com nomes de funções tais como \monoespaco{RE\_new()} e
\monoespaco{RE\_match()} para os métodos ao invés do açúcar sintático
...
\monoespaco{String.charAt} (indexando ao invés de usar ponteiros) ou
\monoespaco{String.substring} (mais próxima da versão com
...
que por sua vez gerou uma função \monoespaco{matchhere}; e assim por
diante.
