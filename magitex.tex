...
\def^^c3#1{\relax% Este \relax é para evitar bugs se usado em \halign
                 % (TeXbook)
        \ifx#1^^bb\ensuremath{{\cal\char`35}}
        \else\ifx#1^^bc\ensuremath@{{}^{\lower0.5ex\hbox{$\scriptstyle1$}}\!/\!_4}
        \else\ifx#1^^bd\ensuremath@{{}^{\lower0.5ex\hbox{$\scriptstyle1$}}\!/\!_2}
        \else\ifx#1^^be\ensuremath@{{}^{\lower0.5ex\hbox{$\scriptstyle3$}}\!/\!_4}
        \else\ifx#1^^bf\utf@{\char`75}
        \else\ifx#1^^80\ifmmode{\hbox{\tenit@\accent18A}}\else{\accent18A}\fi% À
...
\def\sublinhado#1{$\setbox0\hbox{#1}\dp0=0pt\m@th \underline{\box0}$}
...
\def\hidewidth{\hskip-1000pt plus 1fill} % for alignment entries that can stick out
...
\def\cleartabs{\global\setbox\tabsyet\hbox{} \setbox\tabs\hbox{}}
\def\settabs{\setbox\tabs\hbox{} \futurelet\next\sett@b}
...
\def\r@@t#1#2{\setbox0\hbox{$\m@th#1\sqrt{#2}$}\dimen@\ht0
  \advance\dimen@-\dp0
...
\def\eqalign#1{\hbox{}\,\vcenter{\openup3pt\m@th
  \ialign{\strut\hfil$\displaystyle{##}$&$\displaystyle{{}##}$\hfil
      \crcr#1\crcr}}\,}
\newif@\ifdt@p
\def\displ@y{\global\dt@ptrue\openup3pt\m@th
...
\def\eqalignno#1{\displ@y \tabskip0pt plus 1000pt minus 1000pt %Centraliza
  \halign to\displaywidth{\hfil$\@lign\displaystyle{##}$\tabskip0pt plus0pt minus0pt
    ...
    1000pt % Centraliza
    &\llap{$\@lign##$}\tabskip0pt plus0pt minus0pt\crcr
    #1\crcr}}
