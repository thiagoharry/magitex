...
\countdef\llx@=12\countdef\lly@=13\countdef\urx@=14\countdef\ury@=15
\countdef\largura@=16\countdef\altura@=17
...
\def\quebra{\ifvmode\vfill\fi\ifhmode\hfill\fi\penalty-10000}
...
{%
   ...
   \ifx#1\epsfpercent@           % then first char is percent (quick test)
       \ifx \testit \epsfbblit@  % then (slow test) we have %%BoundingBox
            \epsfgrab@ #3 . . . \\%
            \ifx \llx@\epsfatend@ % then ignore %%BoundingBox: (atend)
                \gdef\epsfatend@{1}
...
\def\imagem#1{%
  % Valores padronizados para tamanho da imagem se não conseguirmos
  % descobrir lendo o arquivo:
  \llx@=72% Coordenada x do canto inferior esquerdo
  \lly@=72% Coordenada y do canto inferior esquerdo
  \urx@=540% Coordenada x do canto superior direito
  \ury@=720% Coordenada y do canto superior direito
  \openin0=#1
  \ifeof0%
    \erro@{Arquivo #1 inexistente}%
  \else{% Vamos procurar por informação sobre o tamanho da imagem
        % dentro do arquivo. Para isso temos que ativar um modo verbatim
        % que deixe quase todos os caracteres sem significados especiais
       \catcode`\\=12\catcode`\{=12\catcode`\}=12\catcode`\$=12%
       \catcode`&=12\catcode`\#=12\catcode`\^=12\catcode`\^^K=12\catcode`\_=12%
       \catcode`\^^A=12\catcode`\%=12\catcode`\~=12\catcode`\ =10
       \gdef\epsfileok@{1}\gdef\epsfatend@{0}%
       \loop@
         \read0 to \linha
         \ifeof0
           \gdef\epsfileok@{0}
         \else
           \expandafter\epsfaux@\linha :. \\%
         \fi
         \ifx\epsfileok@1
         \repeat@}%
         \closein0
  \fi%
  % Calculando tamanho da imagem:
  \altura@=\ury@\advance\altura@ by -\lly@%
  \largura@=\urx@\advance\largura@ by -\llx@%
  \ifvmode\noindent\fi % Sair do modo vertical
  \hbox{{\vbox to \altura@ bp{\vfil\hbox to \largura@ bp{%
     \llx@ \lly@ \urx@ \ury@
     %\special{PSfile=#1 llx=\llx@ lly=\lly@ urx=\urx@ ury=\ury@ rwi=10bp rhi=10bp}%
  \hfil}}}}}
